...
const triangleSmall = document.getElementById("triangle-small");
const triangleLarge = document.getElementById("triangle-large");
const triangleGuide = document.getElementById("triangle-guide"); // Triángulo guía
...
function handleMotion(event) {
    ...
    let maxX = gameAreaWidth / 2 - 100 + 25; // Ajuste para triángulos
    let maxY = gameAreaHeight / 2 - 75 + 25; // Ajuste para triángulos

    positionX = Math.max(-maxX, Math.min(maxX, positionX));
    positionY = Math.max(-maxY, Math.min(maxY, positionY));

    // Mover el triángulo pequeño en la pantalla
    triangleSmall.style.transform = `translate(${positionX}px, ${positionY}px)`;

    // Verificar si el triángulo pequeño está en el centro del triángulo guía
    ...
    if (distanceX < threshold && distanceY < threshold && !isGameWon) {
        // Cambiar los colores de los triángulos a verde
        triangleLarge.style.borderBottomColor = 'green';
        triangleSmall.style.borderBottomColor = 'green';
        triangleGuide.style.borderBottomColor = 'green'; // Cambiar el color del triángulo guía también
